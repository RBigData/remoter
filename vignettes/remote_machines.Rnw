%\VignetteIndexEntry{remote-machines}
...
\title{Using remoter with Remote Machines}
\author{Drew Schmidt}
...
\section{Working with Remote Machines}\label{working-with-remote-machines}
...
\subsection{Creating a Remote Server}\label{creating-a-remote-server}
...
\subsection{Connecting to a Server}\label{connecting-to-a-server}
...
\subsection{tunneling Over ssh}\label{tunneling-over-ssh}
...
\subsection{Working with Relays}\label{working-with-relays}
