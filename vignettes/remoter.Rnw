...
install the **sodium** package.  The use of **sodium** is optional because it is
a non-trivial systems dependency, but it is highly recommended.  You can install
it manually with a call to \texttt{install.packages("sodium")} or by installing
\textbf{remoter} via:
